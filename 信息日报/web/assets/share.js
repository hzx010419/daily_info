...
(function () {
  ...
  // 词云尺寸：高度同二维码+20(2x)，宽度为二维码2.25倍
  var CLOUD_W = Math.round(QR_SIZE * 2.25);
  var CLOUD_H = QR_SIZE + 20 * SCALE;
  ...
  var STOP_WORDS = [
    ...
    // 新增：无意义英文词/产品型号（词云过滤）
    'ST','st','IPAD','ipad','iPad','MAC','Mac','mac','MacBook','macbook',
    '法国','德国',
    // 新增：无意义英文缩写
    'VS','vs','ETC','etc','EG','eg','IE','ie','PS','ps','PC','pc','TV','tv',
  ...
  function extractKeywords(data) {
    ...
    for (var i = 0; i < clues.length; i++) {
      ...
      for (var t = 0; t < topics.length; t++) {
        var tp = topics[t];
        // 增加停用词过滤（原代码已有 STOP_WORDS 检查，这里确保也过滤小写形式）
        if (tp && STOP_WORDS.indexOf(tp) === -1 && STOP_WORDS.indexOf(tp.toLowerCase()) === -1) {
    ...
    var ENTITY_WORDS = [
      ...
      // 国际（只保留核心国家/地区，泛化国家如"法国""德国"已移除）
      '美国','中国','欧盟','俄罗斯','乌克兰','以色列','伊朗','日本','印度',
      '达沃斯','硅谷','华尔街',
      // 政策/社会
